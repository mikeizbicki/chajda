...
def test__lemmatize(test,benchmark):
    benchmark(pspacy.lemmatize,test['lang'],test['text']*10)
    #benchmark(pspacy.lemmatize,test['lang'],test['text'],False,False,False,False)


#@pytest.mark.parametrize('test', inputs, ids=[ input['lang'] for input in inputs ])
#def test__lemmatize2(test,benchmark):
    #benchmark(pspacy.lemmatize,test['lang'],[test['text']],False,False,False,False)
#
#
#@pytest.mark.parametrize('test', inputs, ids=[ input['lang'] for input in inputs ])
#def test__lemmatize_batch(test,benchmark):
    #benchmark(pspacy.lemmatize,test['lang'],[test['text']]*1000,False,False,False,False)
