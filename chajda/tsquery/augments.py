...
chajda_path = os.path.dirname(__file__)
...
def create_annoy_index_if_needed(lang, word='google', n=5):
    '''
    Checks whether an AnnoyIndex has already been created for a specified language.
    Populates an AnnoyIndex  with vectors from the fasttext model that corresponds to language,
    builds the index, saves it to disk. 
    The AnnoyIndex is stored in the annoy_indices dictionary.
    ...
    try:
        ...
        try:
            annoy_indices[lang].load(f"{chajda_path}/annoy/{lang}.ann")
        ...
        # OSError occurs if index is not already saved to disk
            ...
            # Populate AnnoyIndex with vectors from corresponding fasttext model
            i = 0
            ...
            try:
                annoy_indices[lang].save(f"{chajda_path}/annoy/{lang}.ann")
            except OSError:
                os.mkdir(f"{chajda_path}/annoy")
                annoy_indices[lang].save(f"{chajda_path}/annoy/{lang}.ann")


def load_fasttext_model(lang, word='google', n=5):
    '''
    ...
    # >>> load_fasttext_model('en') or fasttext_models['en'] != None
    # True
    '''
    ...
    with suppress_stdout_stderr():
        ...
        try:      
            os.chdir(f"{chajda_path}/fasttext")
        except OSError:
            os.mkdir(f"{chajda_path}/fasttext")
            os.chdir(f"{chajda_path}/fasttext")
